...
const API_URL = import.meta.env.VITE_API_URL + "/api/purchases"; // El backend usa Render [14, 15]
...
export const getPurchases = async () => {
    try {
        ...
        const purchases = response.data.map((p: any) => {
            ...
            // ************ INICIO DE CORRECCIÓN DE MAPEO ************
            
            // 1. Obtener la lista de detalles, si existe. Si no, un array vacío.
            const detailsList = Array.isArray(p.details) ? p.details : [];

            // 2. Tomar el PRIMER detalle (objeto) para la vista de tabla simplificada.
            // Si la lista está vacía, usamos un objeto vacío {} para evitar errores de tipo.
            const singleDetail = detailsList.length > 0 ? detailsList : {};

            // 3. Acceder a las propiedades de PurchaseDetailDTO desde el objeto individual
            const productName = singleDetail.productName ?? "Sin producto";
            
            // supplierName debería venir del PurchaseDTO (p.supplierName) si el mapper fue extendido,
            // si no, debe venir del ID mapeado a nivel superior (p.supplierId)
            const supplierName = p.supplierName ?? "Sin proveedor"; 
            
            // Quantity (amount) y Unit Price (mapeado de priceShopping)
            const quantity = Number(singleDetail.amount ?? 0);
            const unitPrice = Number(singleDetail.unitPrice ?? 0); 
            
            // 4. Calcular el total (si no viene directamente del DTO, aunque p.total existe)
            ...
            return {
                ...
                quantity,
                unitPrice, // Aseguramos que unitPrice se pasa al componente [16]
                total,
                date,
            };
        });
        ...
        // Propaga el mensaje de error descriptivo del backend [10, 17]
        throw new Error(

...
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"; [1, 4]
...
const { data } = await getSales(); // data es List<SaleDTO> 
    const mapped: Sale[] = data.map((item) => {
        // La información del producto y precio ahora está en el primer detalle del SaleDetailDTO [11]
        // item.details es una lista. Asumimos que para la vista de tabla, tomamos el primer detalle.
        const detail = item.details && item.details.length > 0 ? item.details : {};

        // Los campos productName y unitPrice fueron añadidos al SaleDetailDTO [11]
        const productName = detail.productName ?? "Producto Desconocido";
        // unitPrice (BigDecimal en Java) [11] se recibe y se valida contra null para evitar $ NaN
        const unitPrice = detail.unitPrice ?? 0;
        // amount en DTO [11] es quantity en Front
        ...
        return {
            ...
            total: item.total, 
            date: item.date,
            // customerName se mapeó directamente en SaleDTO desde user.name [12]
            customer: item.customerName ?? "Cliente", 
        };
    });

    setSales(mapped); [13, 14]
    setFilteredSales(mapped); [13, 14]
} catch (error) {
    toast({ 
...
if (field === "quantity" || field === "unitPrice") { 
    // Cálculo de total seguro para números
